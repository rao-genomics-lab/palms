...
TEMPLATE_NOTES: dict[str, TemplateNote] = {
    ...
    "qc.filter": TemplateNote(
        ...
        reading=(
            ...
            "Work recorded after this node depends on it; work recorded "
            "before it does not, and is not marked stale — it was computed on "
            "every cell and still was. The node is a *barrier*: it rebinds "
            "`adata`, which every earlier step read, so the notebook places "
            "it after all of them and before its own dependents without any "
            "edge saying so. Re-running it with different cutoffs flags only "
            "the filtered lineage. Reverting records nothing, because a "
            "notebook without a filter simply never filtered; the node stays "
            "while any result depends on it."
        ),
        variants=(
