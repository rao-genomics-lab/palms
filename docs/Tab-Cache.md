...
- **A cache-only dataset is never rebuilt, and Force Rebuild is disabled for it.** A [Crop Dataset](Tab-Crop-Dataset) export has no raw Xenium output beside its cache — no `cells.zarr.zip`, no `cell_feature_matrix`, no `morphology_focus` — so the cache *is* the data rather than a derivative of it. The header says so, freshness is reported as not applicable, and the loader will not move the store aside for a rebuild that cannot happen. Verify, Re-consolidate and Recover from Backup all still work, and are the right tools if such a cache is damaged.
