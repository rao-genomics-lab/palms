...
- **Every step here is recorded**, so the analysis appears in `analysis.py` and the exported notebook and replays from the raw Xenium output. The drawn annotations reach the notebook as literal coordinates in the `annotations` cell — it does not need the napari shapes layer. The templates behind the buttons are [annot.polygons](Analysis-Templates#annotpolygons), [annot.virtual_cells](Analysis-Templates#annotvirtual_cells), [annot.nhood](Analysis-Templates#annotnhood) and [annot.nhood_plot](Analysis-Templates#annotnhood_plot), each editable in the [Templates](Tab-Templates) tab.
- Re-running after drawing or deleting a shape re-records the `annotations` step and marks anything computed from the old shapes ⚠ stale in the [Notebook](Tab-Notebook) tab — an analysis over a different set of regions is a different analysis.
