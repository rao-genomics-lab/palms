...
- How each one is carried depends on what its coordinates mean, which the element itself declares: an overlay with its own affine (H&E, ARMS, `ext_*`, `patch_*`) keeps its own pixels and has the crop composed into its transform, while an element already in Xenium pixels (ROIs, annotations, ARMS tiles) has its geometry moved into the crop frame. Either way the drawn region is used to clip, so patches and cell circles outside the crop are dropped rather than shipped.
...
- Each exported dataset is fully standalone: it includes its own `experiment.xenium`, a prebuilt SpatialData zarr cache, `transcripts.parquet`, and a prebuilt transcript feather cache, so it opens at full speed without re-processing. The saved *session* (window state, cached DEG results, the provenance graph) is not inherited; the ROIs, registration and overlays themselves are, as separate elements, along with clusterings (see below).
