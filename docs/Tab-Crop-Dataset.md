...
- Exported datasets have no `analysis/` folder, so there's no separate precomputed UMAP/clustering CSVs to load. If the source dataset's UMAP was already computed and open when you cropped, those coordinates (for the kept cells) are carried over automatically via the cell table and the UMAP tab still works; if not, the UMAP tab starts empty until you compute one.
