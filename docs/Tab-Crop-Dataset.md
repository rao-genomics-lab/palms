...
- `cell_labels` is additionally masked so it only contains cells kept in the exported table. `nucleus_labels` is not: its pixel values are an independent numbering unrelated to `cell_id`/`cell_labels`, so it can't be reliably filtered by ID — it's cropped to the bounding box only and may include nuclei belonging to cells outside the drawn polygon.
...
- Each exported dataset is fully standalone: it includes its own `experiment.xenium`, a prebuilt SpatialData zarr cache, `transcripts.parquet`, and a prebuilt transcript feather cache, so it opens at full speed without re-processing. It does not inherit the source dataset's saved session (ROIs, registration, clustering choices) — cluster labels already stored in the cell table are the exception, since they carry over automatically as part of the cropped cells.
