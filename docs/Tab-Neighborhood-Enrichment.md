...
- The analysis runs on the normalised expression data using the spatial neighbour graph shared across the Spatial tabs. **N neighbors** is therefore a property of that shared graph: the first tab to build it for a given dataset fixes the value, and later tabs reuse it.
