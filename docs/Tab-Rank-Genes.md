...
- One ranking is kept per clustering, so ranking a second clustering does not discard the first. Each is stored under `uns['rank_genes_<clustering>']`, both in the saved dataset and in the exported notebook, so a plot can name the ranking it wants. Datasets saved before this change hold a single unkeyed result and still open normally.
