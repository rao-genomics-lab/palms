...
- **Applying a filter starts a second lineage; earlier results are not marked stale.** A clustering you ran before the filter was computed on every cell and still was, so it keeps its clean badge in the [Notebook](Tab-Notebook) tab and the exported notebook runs it *before* the filter cell, on the full table. Work you do after the filter is recorded under it. Re-running an earlier clustering while filtered revises that one result, and only its dependents go stale. Changing the cutoffs marks stale only what was computed under the filter.
- **Reverting records nothing.** There is no code for "un-filter" — a notebook without a filter simply never filtered — so nothing is added, and what you compute afterwards is recorded above the filter cell again, where the notebook still has every cell. The filter step itself stays in the notebook while any result depends on it (it is what makes that result reproducible) and is removed once nothing does. Re-applying the same cutoffs later marks nothing stale.
