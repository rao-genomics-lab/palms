...
Usage::
    ...
    xenium-rename-dataset /data/new_name --repair      # fix an already-moved one
...
from xenium_viewer.utils.adata_persistence import sidecar_dir  # noqa: E402
from xenium_viewer.utils.zarr_safe import atomic_json, safe_group_update, store_lock  # noqa: E402
...
def preflight(data_path: Path, dest: Path | None, *, dry_run: bool) -> None:
    ...
    from xenium_viewer.utils.zarr_safe import recover_pending
    ...
    try:
        ...
        if exc.errno == errno.EXDEV:
            raise PreflightError(
                ...
                f"    xenium-rename-dataset {dest} --repair --from {data_path}"
            ) from exc
        raise
