...
def build_tab(ctx: ViewerContext) -> tuple:
    ...
    def _on_load():
        ...
        def _on_done(result):
            ...
            try:
                ...
                from xenium_viewer.utils.adata_persistence import save_custom_seg_to_sdata
    ...
    def _restore_session(session):
        if session.get("segmentation_source") != "custom":
            return
        from xenium_viewer.utils.adata_persistence import load_custom_seg_from_sdata
        adata, scales = load_custom_seg_from_sdata(ctx.sdata)
        if adata is None:
            active_label.setText(
