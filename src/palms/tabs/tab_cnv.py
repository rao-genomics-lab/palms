...
from palms.tabs._helpers import make_tab, StatusProxy, attach_spinner, make_progress_bar, combo_value_kwargs
...
def _resolve_copykat_prefix() -> "Path | None":
    ...
    detached worker runs in a second env (default name ``palms_copykat``,
    ...
    the system R). Resolution order:
      1. ``PALMS_COPYKAT_PYTHON`` — path to that env's python (prefix inferred);
      2. a sibling conda env named by ``PALMS_COPYKAT_ENV`` (default
         ``palms_copykat``) under the same conda base as this env.
    Returns the env prefix Path, or ``None`` if not found.
    """
    explicit = os.environ.get("PALMS_COPYKAT_PYTHON")
    if explicit and Path(explicit).exists():
        return Path(explicit).resolve().parents[1]  # <prefix>/bin/python -> <prefix>
    env_name = os.environ.get("PALMS_COPYKAT_ENV", "palms_copykat")
...
def build_tab(ctx: ViewerContext) -> tuple:
    ...
    def _record_cnv_node(result):
        ...
        second conda env (``palms_copykat``), because its R stack needs
        ...
        L = [
            ...
            "# 'palms_copykat' conda env (R + rpy2 need python 3.11), so",
    ...
    def _ingest_cnv_result(result, note=""):
        ...
        from palms.utils.adata_persistence import save_clustering_to_adata, save_cnv_results_to_adata
    ...
    def _launch_copykat(reference_key, reference_ids, analyze_ids, analyze_categories):
        ...
        if copykat_prefix is None:
            return False, (
                "CopyKAT backend needs the separate 'palms_copykat' env. Create it with: "
                "conda env create -f environment-copykat.yml  (or set PALMS_COPYKAT_PYTHON)."
            )
        conda_exe = os.environ.get("CONDA_EXE") or "conda"
        from palms.utils.cnv_analysis import subsample_indices
        ...
        # Make THIS repo's palms importable there via PYTHONPATH, so the
        ...
        proc = subprocess.Popen(
            [conda_exe, "run", "-p", str(copykat_prefix), "--no-capture-output",
             "python", "-m", "palms.cnv_copykat_worker",
